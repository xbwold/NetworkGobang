	...
	public void run() {
		try {
			while (true) {
				...
				if (dataArr[0].equals("0")||dataArr[0].equals("7")||dataArr[0].equals("8")) {	//0棋子信息、7悔棋信息、8对手人数信息 直接转发
					int id=getSocketId(client);
					sendMessageById(id, data);
				} else if (dataArr[0].equals("1")) {	//房间内聊天信息
					int id=getSocketId(client);
					sendMessageById(id,data);
				} else if (dataArr[0].equals("2")) {	//接收准备信息
					int id=getSocketId(client);
					sendMessageById(id, data);
				} else if (dataArr[0].equals("3")) {// 游戏大厅聊天信息
					System.out.println(data);
					sendMessageAllUser(data);
				} else if (dataArr[0].equals("4")) { // 创建房间信息
					homeList.add(data);
					...
					if(dataArr[1].equals("1")) { //房主退出，解散房间
						for(int i=0;i<2;i++) {	//用于开启游戏大厅点击事件
							String message="9:0";
							if(i==0) {
								message+=":0";
							}else {
